...
fn create_redb_table(n: usize, name: &str) -> (Arc<RedbTable>, tempfile::TempDir) {
    ...
    // Insert N rows
    for i in 0..n {
        storage.insert(i as i64, format!("value_{}", i).as_bytes()).unwrap();
    }
...
async fn test_learned_index_point_query_performance() {
    ...
    println!("Average point query time (10K rows, 10 iterations): {:.3}ms", avg_time * 1000.0);

    // Point query should be reasonably fast (< 100ms average is acceptable for testing)
    assert!(avg_time < 0.1, "Point query should be < 100ms, got {:.3}ms", avg_time * 1000.0);
}
...
async fn test_learned_index_vs_full_scan_speedup() {
    ...
    let point_query_time = measure_query_time(
        &ctx,
        "SELECT * FROM speedup_test WHERE id = 2500",
        5
    ).await;
    ...
    println!("Point query (learned index): {:.3}ms", point_query_time * 1000.0);
    ...
    if speedup >= 1.5 {
        ...
        println!("⚠ Learned index comparable to full scan ({:.2}x) - expected on small datasets", speedup);
    } else {
        println!("❌ Learned index significantly slower ({:.2}x) - investigating", speedup);
    }

    // Just verify both queries work correctly (no speedup requirement on 5K rows)
    assert!(point_query_time < 1.0, "Point query should complete in reasonable time");
    assert!(full_scan_time < 1.0, "Full scan should complete in reasonable time");
}
...
async fn test_learned_index_multiple_point_queries() {
    ...
    for id in test_ids {
        let start = Instant::now();
        let df = ctx.sql(&format!("SELECT * FROM multi_point_test WHERE id = {}", id)).await.unwrap();
    ...
    println!("Average time across 5 different point queries: {:.3}ms", avg_time * 1000.0);

    // Each point query should be reasonably fast regardless of position
    assert!(avg_time < 0.1, "Point queries should average < 100ms, got {:.3}ms", avg_time * 1000.0);
}
...
async fn test_learned_index_scaling() {
    ...
    assert!(time_ratio < 5.0, "Learned index should scale sub-linearly, got {:.2}x slowdown for {}x data", time_ratio, size_ratio);
}
...
async fn test_learned_index_miss_performance() {
    ...
    assert!(miss_time < 0.1, "Learned index misses should be < 100ms, got {:.3}ms", miss_time * 1000.0);
}
...
async fn test_learned_index_range_query_benefit() {
    ...
    assert!(small_range_time < 0.05, "Small range queries should be < 50ms, got {:.6}s", small_range_time);
}
...
async fn test_learned_index_aggregation_with_filter() {
    ...
    println!("Filtered aggregation time: {:.3}ms", filtered_agg_time * 1000.0);

    // Should benefit from learned index for filtering
    assert!(filtered_agg_time < 0.1, "Filtered aggregation should be fast, got {:.3}ms", filtered_agg_time * 1000.0);
}
...
async fn test_learned_index_consistency() {
    ...
    for i in 0..100 {
        let df = ctx.sql(&format!("SELECT * FROM consistency_test WHERE id = {}", i * 10)).await.unwrap();
        let results = df.collect().await.unwrap();

        if results[0].num_rows() > 0 {
            // Verify ID matches query
            let id_array = results[0].column(0)
                .as_any()
                .downcast_ref::<arrow::array::Int64Array>()
                .unwrap();

            assert_eq!(id_array.value(0), i * 10, "Learned index returned wrong row");
        }
    }
}
...
async fn test_learned_index_vs_baseline_benchmark() {
    ...
    let point_time = measure_query_time(
        &ctx,
        "SELECT * FROM benchmark WHERE id = 2500",
        5
    ).await;
    ...
    for i in (0..5_000).step_by(500) {
        let df = ctx.sql(&format!("SELECT * FROM benchmark WHERE id = {}", i)).await.unwrap();
        let _results = df.collect().await.unwrap();
    }
    let multi_point_time = start.elapsed().as_secs_f64() / 10.0;
    println!("Multi-point query average (10 queries): {:.3}ms", multi_point_time * 1000.0);
    ...
    println!("Speedup (full scan / point query): {:.1}x", full_scan_time / point_time);
    println!("=========================================\n");

    // Performance assertions (realistic targets)
    assert!(point_time < 0.1, "Point query should be < 100ms, got {:.3}ms", point_time * 1000.0);
    assert!(multi_point_time < 0.1, "Multi-point queries should average < 100ms, got {:.3}ms", multi_point_time * 1000.0);
    ...
    if point_time > 0.001 {
        ...
        if speedup < 2.0 {
            println!("WARNING: Speedup below 2x - learned index may not be providing expected benefit");
        }
    }
}
